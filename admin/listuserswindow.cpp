    ...
    mUser_id{user_id}
{
    ...
    QVector<QPair<int, User>> users = db::get_users();
    ...
    for (int i = 0; i < users.size(); ++i) {
        ...
        if (users[i].second.role() == UserRole::Client)
            ui->users_table_widget->setItem(i, 1, new QTableWidgetItem{QString::number(db::get_client_orders_count(users[i].first))});
        else if (users[i].second.role() == UserRole::Worker)
            ui->users_table_widget->setItem(i, 1, new QTableWidgetItem{QString::number(db::get_worker_orders_count(users[i].first))});
        else
...
void ListUsersWindow::on_delete_button_clicked()
{
    try {
        ...
        if (confirm_dialog.exec() == QDialog::Rejected)
            return;
        QSharedPointer<QPair<int, User>> selected_user = db::get_user(this->ui->users_table_widget->item(current_row, 2)->text());
        if (!selected_user)
            throw std::runtime_error{"Не удалось получить данные о пользователе"};
        if (!db::delete_user(selected_user->first))
