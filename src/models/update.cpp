...
bool db::update_user_name(const int& user_id, const QString& new_name) {
...
bool db::update_user_login(const int& user_id, const QString& new_login) {
    QSqlQuery query{db::current_pool()};
...
bool db::update_user_role(const int& user_id, const UserRole& new_role) {
    QSqlQuery query{db::current_pool()};
...
bool db::update_user_password(const int& user_id, const QString& new_password) {
    QSqlQuery query{db::current_pool()};
...
bool db::update_order_executor(const int& order_id, const int& new_executor_id) {
    QSqlQuery query{db::current_pool()};
...
bool db::complete_order(const int& order_id, const QString& description) {
    QSqlQuery query{db::current_pool()};
...
bool db::update_passwords(const QString& password) {
    QSqlQuery query{db::current_pool()};
    query.prepare("UPDATE users SET password = :password");
    query.bindValue(":password", User::hash_password(password));
    return query.exec();
}
